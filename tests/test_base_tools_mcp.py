...
@pytest.mark.anyio
async def test_bfsnr_for_gemeinde_mcp_inexisting_gemeinde(not_existing_gemeinde, mcp_session):
    result = await mcp_session.call_tool(
...
@pytest.mark.anyio
async def test_egrid_for_address_mcp_inexisting_address(not_existing_address, mcp_session):
    result = await mcp_session.call_tool(
